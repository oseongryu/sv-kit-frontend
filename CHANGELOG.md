...
0.26.0 의 주석·기록을 사실대로 고친 것뿐이다(동작 동일) — `overflow-hidden` 으로는
tailwind-merge 가 기본값을 못 덮는다고 적었는데 실제로는 덮는다. 고른 클래스는 그대로다.
...
- 뼈대 클래스는 `DialogContent` 기본값(`grid`·`max-h-[calc(100dvh-2rem)]`·
  `overflow-y-auto`)을 tailwind-merge 로 덮는다 — 스크롤을 갖는 자리는 본문 하나뿐이다.
...
소비 앱 이름을 적던 옛 항목들도 "소비 앱" 으로 일반화했다(공개 리포라 남의 리포 이름을 남기지 않는다).
...
넓은 화면에서는 아무것도 하지 않는다.

"목록에서 고르면 닫는다"를 화면들이 `layoutRef.close()` 로 적고 있었는데, `close()` 는
넓은 화면에서 **패널 접기**다. 좁은 화면에서 목록이 상세를 덮는 사정이 넓은 화면에는
없으므로, 나란히 놓인 화면에서 고를 때마다 목록이 접혀 다음 항목을 고르려면 매번
머리줄 토글을 눌러야 했다.

- `close()`·`open()`·`toggle()`·`isClosed()` 는 그대로다 — 손잡이에 한 가지가 늘었을 뿐
- 머리줄 버튼은 계속 `toggle()` 을 쓴다(넓은 화면에서 목록을 일부러 접는 건 여전히 가능)
- 소비 화면은 선택 시 호출을 `close()` → `closeIfNarrow()` 로 바꾸면 된다.
  안 바꿔도 종전 동작 그대로다

화면이 `matchMedia` 로 폭을 다시 가르지 않게 하는 것이 목적이다 — 브레이크포인트
(`mobileBreakpoint`, 기본 767)를 아는 곳은 이 컴포넌트 하나로 유지한다.

## 0.19.1

**`ui/split-layout` 이 넓은 화면에서 목록이 접힌 채 뜨던 문제를 고쳤다.** 폭을 기억하는
가드가 상수(`>5 && <95`)였는데, 화면은 `leftMinSize={10}` 처럼 더 큰 최소 폭을 준다.
그 사이 구간(6~9%)까지 목록을 좁게 끌면 값이 저장되고, 다음 마운트에서 `defaultLayout`
의 왼쪽이 `minSize` 미만이라 접히는 패널이 `collapsedSize=0` 으로 스냅됐다 — 목록이
사라진 것처럼 보이고, 머리줄 토글을 눌러야 돌아왔다.

이제 저장·복원 양쪽이 화면이 준 `[leftMinSize, 100 - rightMinSize]` 를 같은 자로 쓴다.

- 범위 밖 값은 **저장하지 않고, 이미 저장돼 있으면 복원하지 않는다** — 지우지는 않는다.
  그래서 소비자는 localStorage 를 손대지 않아도 올리기만 하면 정상으로 돌아온다
- props·기본값은 그대로다. 폭을 그 범위 안에서 쓰던 사용자는 달라지는 게 없다

증상이 넓은 화면에서만 났던 이유는 좁은 화면이 서랍(`Sheet`) 경로라 패널 접기를 타지
않기 때문이다. 폭은 `storageKey` 별로 저장되므로 그렇게 끌어 둔 화면 하나에서만 났다.
...
`20260804.*` 세 판은 실제로 배포됐으므로 아래 항목과 태그를 그대로 두고, **이 판부터**
다시 semver 다 — 그래서 되돌림인데도 번호는 뒤로 가지 않고 0.18.1 의 다음 patch 다.
...
- `20260804.3.0` 에서 예제 `package-lock.json` 을 버전 관리에서 뺀 것은 **유지한다.**
  그 결정은 버전 체계와 무관하게 맞았다(lock 은 태그가 원격에 올라간 뒤에야 갱신되는데
  그 변경을 커밋하려면 또 판을 올려야 하는 순환이 있었다)
- 릴리스 절차에서 `SKELETON_IMPL.md` 갱신 단계를 되살렸다 — `20260804.2.0` 에서 lock
  재생성 단계를 끼워 넣으며 같이 지워졌던 것이고, 지울 이유가 있던 변경이 아니다
- 코드 변경 없음 — 버전 문자열과 문서(README 릴리스 절차·CONTRACT 배포·버전 규약)뿐

## 20260804.3.0

- 예제의 `package-lock.json` 을 버전 관리에서 뺐다. 앞 판에서 lock 을 재생성했더니
  **그 변경을 커밋하려면 또 판을 올려야 하는 순환**이 드러났다 — lock 은 태그가 원격에
  올라간 뒤에야 갱신되기 때문이다. 예제는 배포물이 아니라 복붙용 참고라 lock 을 고정할
  실익이 없다. 릴리스 절차에서도 그 단계를 뺐다
...
(이 결정은 0.18.2 에서 되돌렸다. 그래도 이 판을 포함한 `20260804.*` 세 판은 배포된
사실이라 그대로 둔다. 예제 핀을 함께 올리는 릴리스 절차 단계는 그대로 살아 있다.)
